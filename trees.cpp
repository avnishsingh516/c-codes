...
L1 balanceopti( node* root)
{
	...
	L1 left = balanceopti(root->left);
	L1 right = balanceopti(root->right);

	val.height = max(left.height,right.height) + 1;

	int gap=abs(left.height-right.height);

	if (gap<=1  &&  left.balanceopti && right.balanceopti)
	{
		val.balnced=1;
		return val;
	}
...
int main(){
	...
	L1 pair = balanceopti(root);
	cout<<pair.balnced<<endl;
